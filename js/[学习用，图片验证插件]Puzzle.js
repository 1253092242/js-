	    ...
	    var SliderBar = function (targetDom, options) {
	        ...
	        if (dataList.length > 0) {
	            var html = "<div class='slide-box'><div class='slide-img-block'>" +
	                ...
	                "<div class='slide-title' id='slideHint'> <<<完成拼图获取验证码</div></div></div>";
...
//	            }
	    ...
	    SliderBar.prototype = {
	        ...
	        reToNewImg: function () {
	            var _this = this;        // 该方法有等于0 的情况
	            var imgSrc = "./images/2.jpg" ;
	        ...
	        mousedown: function (_this, e) {
	            ...
	            function mouseup() {
	                ...
	                if(_this.resultX > (left - 2) && _this.resultX < (left + 2)){
						...
						$("#slideBar").hide();
	                    $("button:first").attr("isok","no");
						$("button:first").attr('disabled','disabled');
						$("button:first").text("60");
						$.ajax({
			        		type:"post",
			        		url:"verificationCode/singleSendercode.do",
			        		data:{'phonenumber':$("#moble").val(),"type":type},
			    			dataType: "json",
			        		success:function(e){
			        			alert("验证码已发送");
			        			console.log($("#moble").val())
			        			console.log(type)
			        		},
			        		eerror:function(e){
			        		 alert("验证码发送出错，请联系管理员");
			        		}
