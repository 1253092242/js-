 ...
 */
function isPlainObject (obj) {//用于检测是否是纯粹的对象
...
function makeMap (
  ...
  return expectsLowerCase
    ? function (val) { return map[val.toLowerCase()]; }//toLowerCase用于转换为小写
...
function checkComponents (options) {
  for (var key in options.components) {//循环组件的数组
    validateComponentName(key);
  }
}

function validateComponentName (name) {
  if (!/^[a-zA-Z][\w-]*$/.test(name)) {//正则检测名称是否含有无法识别的字符
    warn(
  ...
  if (isBuiltInTag(name) || config.isReservedTag(name)) {//前者用于检测是否含有内置组件，后者用于检测是否与html内置dom名和svg一样
    warn(
 ...
 * Object-based format.
 */
function normalizeProps (options, vm) {//统一props
  ...
  if (Array.isArray(props)) {
    ...
    while (i--) {
      ...
      if (typeof val === 'string') {
        name = camelize(val);//把名称改为驼峰式
        res[name] = { type: null };
      } else {
        warn('props must be strings when using array syntax.');
      }
    }
  } else if (isPlainObject(props)) {//是不是纯粹的对象，排除dom数组等对象
...
function mergeOptions (parent,child, vm){
  {
    checkComponents(child);//会循环检测对象内部的组件是否可用
  }
  ...
  normalizeProps(child, vm);////统一props，让props更加规范
...
function initLifecycle (vm) {
  ...
  vm.$root = parent ? parent.$root : vm;//根实例，没有就是自己

  vm.$children = [];//子组件
  vm.$refs = {};//挂载组件

  vm._watcher = null;//watcher 实例对象。
  vm._inactive = null;//keep-alive组件状态
  vm._directInactive = false;//同上
  vm._isMounted = false;//是否完成挂载
...
function defineComputed (
  ...
  var shouldCache = !isServerRendering();//跳过，看环境的貌似
...
function initMixin (Vue) {
  Vue.prototype._init = function (options) {
    ...
    vm._isVue = true;
    // merge options
    if (options && options._isComponent) {//是否是已注册组件？
      ...
      vm.$options = mergeOptions(//检查option元素是否合规，合并构造器和新实例的options属性
    ...
    /* istanbul ignore else */
    {
      initProxy(vm);//创建代理？添加一个_renderProxy的属性 暂时不知道干嘛用
    }
    // expose real self
    vm._self = vm;//这又是干嘛用？
    initLifecycle(vm);//创建生命周期
    initEvents(vm);
    initRender(vm);
...
function initExtend (Vue) {
  ...
  Vue.extend = function (extendOptions) {
    ...
    var SuperId = Super.cid;//当前组件构造器的id
    var cachedCtors = extendOptions._Ctor || (extendOptions._Ctor = {});//是否已注册，有没有父组件,没有则为空
    if (cachedCtors[SuperId]) {//如果当前superId的构造器已存在，则直接返回构造器组件的id
